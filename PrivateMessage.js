    ...
    const sendMessage = async () => {
      ...
      useEffect(() => {
        const fetchSenderAndMessage = async () => {
          try {
            if (!auth || !auth.currentUser) {
              setTimeout(fetchSenderAndMessage, 1000);
              return;
            }
            ...
            messagesSnapshot.forEach((doc) => {
              ...
              if (recipient === recipient && sender === loggedInUsername) {
      ...
      return (
        <View>
          {messages.map((senderData, index) => (
            <View key={index}>
              {senderData.message && (
                  <View style={{ backgroundColor: 'white', padding: 10, marginVertical: 5 }}>
                    <Text>{senderData.message}</Text>
                  </View>
                                )}
            </View>
          ))}
